...

    {
        ...
        generator = thread::Builder::new()
            ...
            .spawn(move || {
                for _ in 0..epochs {
                    let mut id: String =
                        String::from("INVALID_ID_FIRST_ENTRY_YOU_SHOULD_NOT_SEE_THIS");
                    ...
                    while let Ok(bytes_read) = reader.read_until(b'\n', &mut buffer) {
                        if bytes_read == 0 {
                            ...
                            // Submit the last sequence (or in the case of some genomes, the entire
                            // sequence)
        ...
        if let Ok(command) = rawseq_queue.pop() {
            ...
            for (start_coords, end_coords) in coords {
                if (end_coords - start_coords) >= kmer_size {
                    jobs.fetch_add(1 as usize);
                    let wp = ThreadCommand::Work(Sequence {
                        rawseq: rawseq[start_coords..end_coords].to_vec(),
                        id: id.clone(),
                    });
