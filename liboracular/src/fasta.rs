use crate::threads::{
    sequence_generator, Sequence, SequenceBatch, SequenceBatchKmers, SequenceKmers,
    SequenceTargetContexts, ThreadCommand,
};
...
/// the FASTA ID, a target kmer (in the middle of window_size kmers) and context
/// kmers meant to be used in the learning process.
///
/// So given a k=2 and window_size of 4
/// AC TG GC AC TG GC AC TG TT AA CC GG TT
///
/// Will result in TG as a target word, and AC, TG, GC, AC, GC, AC, TG, TT as
/// context words. Followed by the entire window increasing by 1 and repeating,
/// as well as the reverse complement.
///
/// SequenceBatch type is Vec<SequenceTargetContexts>
///
/// So this function fills up a buffer with *buffer_size* sets of *batch_size*
/// SequenceTargetContexts, waiting for python or another function to pull from
/// it.
pub fn parse_ctfasta_target_contexts(
    ...
    let unshuffled_queue = Arc::new(ArrayQueue::<ThreadCommand<SequenceTargetContexts>>::new(
        shuffle_buffer,
    ));
    ...
    for _ in 0..num_threads {
        ...
        let child = match Builder::new()
            ...
            .spawn(move || {
                ctfasta_worker_thread(kmer_size, window_size, seq_queue, unshuffled_queue, jobs)
            }) {
            Ok(x) => x,
    ...
    // Need to return things (generator_done, generator, jobs, children) so that
    // other threads/processes can handle them...
...
/// This function does the work of splitting up kmers into target and
/// contexts... and shuffling it off to raw_queue. Reverse complement is handled
/// in io_worker!
        ...
        if let Ok(command) = seq_queue.pop() {
            ...
            for i in 0..end {
                ...
                for z in 0..window_size {
                    contexts.push(
                        String::from_utf8(kmers[window_size + z].to_vec())
                            .expect("Invalid UTF-8 Encoding"),
                    );
                }

                target =
                    String::from_utf8(kmers[window_size].to_vec()).expect("Invalid UTF-8 Encoding");
...
pub fn parse_fasta_kmers_shuffle(
    ...
    let unshuffled_queue = Arc::new(ArrayQueue::<ThreadCommand<SequenceKmers>>::new(
        shuffle_buffer,
    ));
    let batch_queue = Arc::new(ArrayQueue::<ThreadCommand<SequenceBatchKmers>>::new(
        buffer_size,
    ));
    ...
    for _ in 0..num_threads {
        ...
        let child = match Builder::new()
            ...
            .stack_size(WORKERSTACKSIZE)
            .spawn(move || {
                fasta_kmers_worker_thread(kmer_size, sample_size, seq_queue, unshuffled_queue, jobs)
            }) {
            Ok(x) => x,
